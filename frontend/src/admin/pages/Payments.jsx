...
export default function Payments() {
  ...
  const stats = useMemo(() => {
    ...
    return { totalRevenue, successful, pending, platformEarnings };
  }, [payments]);
  ...
  return (
    <div className="space-y-6 bg-[#F8FAFC] p-4 sm:p-6">
      {/* ─── Header ────────────────────────────────────────────── */}
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-6 flex-wrap">
          <div>
            ...
            <p className="text-lg font-bold text-blue-600">{formatCurrency(escrowStats?.held || 0)}</p>
          </div>
          <div>
            <span className="text-xs text-gray-500 uppercase">Released</span>
            <p className="text-lg font-bold text-green-600">{formatCurrency(escrowStats?.released || 0)}</p>
          </div>
          <div>
            <span className="text-xs text-gray-500 uppercase">Refunded</span>
            <p className="text-lg font-bold text-gray-600">{formatCurrency(escrowStats?.refunded || 0)}</p>
          </div>
          <div>
            <span className="text-xs text-gray-500 uppercase">Platform Fees</span>
            <p className="text-lg font-bold text-orange-600">{formatCurrency(escrowStats?.platform_fees || 0)}</p>
          </div>
        </div>
            ...
            <div className="p-6 space-y-6">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <span className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium border ${getEscrowBadge(selectedPayment.escrow_status)}`}>
                    {selectedPayment.escrow_status || 'waiting'}
                  </span>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm font-mono">{selectedPayment.mpesa_receipt || '—'}</p>
                </div>
                <div className="col-span-2">
                  <p className="text-xs font-semibold uppercase text-gray-400">Date</p>
                  <p className="mt-1 text-sm">{formatDate(selectedPayment.created_at)}</p>
                </div>
              </div>
              ...
              <div className="border-t border-gray-100 pt-4">
                <h3 className="font-semibold text-gray-700 mb-3">Waste Details</h3>
                <div className="grid grid-cols-2 gap-4">
                  <div>

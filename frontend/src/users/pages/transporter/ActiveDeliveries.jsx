...
export default function ActiveDeliveries() {
  ...
  // ─── Role error / redirect state ─────────────────────────────
  if (error && error.toLowerCase().includes("transporter")) {
    return (
      <div className="min-h-screen bg-[#F8FAFC] px-4 py-8">
        <div className="mx-auto max-w-2xl rounded-3xl border border-yellow-200 bg-yellow-50 p-8 text-center shadow-sm">
          <AlertCircle className="mx-auto h-14 w-14 text-yellow-600" />
          <h2 className="mt-4 font-display text-2xl font-bold text-yellow-800">
            Access Restricted
          </h2>
          <p className="mt-2 text-yellow-700">{error}</p>
          <p className="mt-4 text-sm text-yellow-600">
            This page is designed for transporters. As a producer, you can track your jobs from the Route Tracking page.
          </p>
          <button
            onClick={() => navigate("/dashboard/routes")}
            className="mt-6 inline-flex items-center gap-2 rounded-2xl bg-[#11402D] px-6 py-3 font-semibold text-white hover:bg-[#0E2A1C]"
          >
    ...
    return (
      <div className="min-h-screen bg-[#F8FAFC] px-4 py-8">
        <div className="mx-auto max-w-xl rounded-3xl border border-red-200 bg-red-50 p-8 text-center shadow-sm">
          ...
          <h2 className="mt-4 font-display text-xl font-bold text-red-700">Something went wrong</h2>
          <p className="mt-2 text-sm text-red-600">{error}</p>
          <button
            onClick={fetchDeliveries}
            className="mt-6 rounded-2xl bg-red-600 px-6 py-2.5 font-semibold text-white hover:bg-red-700"
          >
            Try Again
          </button>
        </div>
